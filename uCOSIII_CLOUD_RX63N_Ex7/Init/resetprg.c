...
*                1. The User Stack Pointer (USP) and Interrupt Stack Pointer (ISP)
*                   are both set immediately after entry to this function.
*                   The USP and ISP stack sizes are set in the file stacksct.h.
*                   Default sizes are USP=4K and ISP=1K.
*                2. The interrupt vector base register is set to point to the 
*                   beginning of the relocatable interrupt vector table.
*                3. The MCU is setup for floating point operations by setting
*                   the initial value of the Floating Point Status Word (FPSW).
*                4. The MCU operating frequency is set by configuring the 
*                   Clock Generation Circuit (CGC) in operating_frequency_set.
*                5. Calls are made to functions to setup the C runtime environment
*                   which involves initializing all initialed data, zeroing 
*                   all uninitialized variables, and configuring STDIO if used
*                   (calls to _INITSCT and _INIT_IOLIB).
*                6. Board-specific hardware setup, including configuring I/O
*                   pins on the MCU, in hardware_setup.
*                7. Global interrupts are enabled by setting the I bit in the
*                   Program Status Word (PSW), and the stack is switched from
*                   the ISP to the USP.  The initial Interrupt Priority Level
*                   is set to zero, enabling any interrupts with a priority
*                   greater than zero to be serviced.
*                8. The processor is optionally switched to user mode.  To run
*                   in user mode, set the macro RUN_IN_USER_MODE above to a 1.
*                9. The bus error interrupt is enabled to catch any accesses 
*                   to invalid or reserved areas of memory.
*
*                Once this initialization is complete, the user's main()
*                function is called.  It should not return.
* Arguments    : none
* Return value : none
******************************************************************************/
uint32_t *myPtr;
