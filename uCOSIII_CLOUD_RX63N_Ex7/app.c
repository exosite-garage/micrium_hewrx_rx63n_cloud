...
static  OS_TCB         BlinkyTaskTCB;
static  CPU_STK        AppTaskStartStk[APP_TASK_START_STK_SIZE];
static  CPU_STK        BlinkyTaskStk[BLINKY_TASK_STK_SIZE];
...
int  main (void)
{
	...
	YRDKRX62N_RSPI_Init();                                       /* Initialize LCD                                     */
    lcd_initialize();
...
static  void  AppTaskStart (void *p_arg)
{
   ...
   (void)&p_arg;
    ...
    App_TCPIP_Init(&net_err);                                    /* Initialize uC/TCP-IP & associated applications.      */
	

    OSTaskCreate((OS_TCB     *)&BlinkyTaskTCB,                  /* Create the start task                                */
                 (CPU_CHAR   *)"Blinky Task",
                 (OS_TASK_PTR ) BlinkyTask,
                 (void       *) 0,
                 (OS_PRIO     ) BLINKY_TASK_PRIO,
                 (CPU_STK    *)&BlinkyTaskStk[0],
